...
# MelissaEmailObjectLinuxJava
#
# Downloads the required components and then compiles, packages, and runs MelissaEmailObjectLinuxJava.
#
...
# sample with javac, packages it into a jar, and runs it against the supplied email address.
#
# Overall flow:
...
#   4. Compile, package, and run (single test email or interactive).
#
# Options:
#   --email <value>     Email address to verify.
...
#
# Examples:
#   ./MelissaEmailObjectLinuxJava.sh --license "your-license"
#   ./MelissaEmailObjectLinuxJava.sh --email "info@melissa.com" --license "your-license"
...
# com/melissadata
Wrapper_FileName="libmdEmailJavaWrapper.so"
...
Com_FileName="mdEmail_JavaCode.zip"
...
{    
    ...
    if [ ! -f "$ProjectPath/mdEmail_JavaCode.zip" ];
    then
        printf "mdEmail_JavaCode.zip not found.\n"
        ...
        then
            unzip "$ProjectPath/mdEmail_JavaCode.zip" -d "$ProjectPath"
        else
...
# Run project
# No email supplied -> run interactively; otherwise pass the email in.
if [ -z "$email" ];
then
